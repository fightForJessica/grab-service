...
@Composable
fun RequestContentItemLayout(request: HttpReq) {
    Column(
        modifier = Modifier
            .fillMaxSize()
        ...
        Row(
            ...
            Text(
                text = stringResource(R.string.request_detail_content),
                fontSize = 18.sp,
        ...
        SingleTextContentItem(
            titleText = stringResource(R.string.request_method),
            contentText = request.method.orNone()
        )

        // HTTP 版本
        SingleTextContentItem(
            titleText = stringResource(R.string.http_version),
            contentText = request.httpVersion.orNone()
        )

        // 请求头
        if (request.formatHead != null && request.formatHead.size >= 2) {
            val headerList = request.formatHead.subList(1, request.formatHead.size).map {
